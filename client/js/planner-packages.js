...
const imageBaseUrlPlanner = "http://localhost:5001";
...
const loadPlannerPackages = async () => {
  try {
    const res = await fetch("http://localhost:5001/api/packages/my-packages", {
      headers: {
...
const deletePackage = async (id) => {
  ...
  try {
    const res = await fetch(`http://localhost:5001/api/packages/${id}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${plannerPackageToken}`,
      },
